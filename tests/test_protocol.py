...
class TestSendPacket:
    """Tests for sending protocol packets."""

    async def test_update_sends_device_status(self, h: ProtocolHarness) -> None:
    ...
    async def test_pair_sends_pairing_request(self, h: ProtocolHarness) -> None:
        """Pair should send a pairing request packet."""
        resp = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 1, TuyaBLECode.FUN_SENDER_PAIR, b"\x00"
            )
        )
        await conclude(h.device.pair(), lambda: h.notify(resp))
        assert len(h.writes()) >= 1

    async def test_packet_encrypted_and_fragmented(self, h: ProtocolHarness) -> None:
...
class TestSendDatapoints:
    """Tests for sending datapoints."""

    async def test_send_datapoints_v3(self, h: ProtocolHarness) -> None:
        ...
        resp = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 1, TuyaBLECode.FUN_SENDER_DPS, b"\x00"
            )
        )
    ...
    async def test_send_datapoints_wrong_protocol_raises(
        self, h: ProtocolHarness
    ) -> None:
...
class TestReceiveDatapoints:
    """Tests for receiving datapoints."""

    def _dp_data(self) -> bytes:
        out = bytearray()
    ...
    async def test_receive_dp_all_types(self, h: ProtocolHarness) -> None:
        ...
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_RECEIVE_DP, self._dp_data()
            )
        )
        h.notify(msg)
        await asyncio.sleep(0)
    ...
    async def test_receive_sign_dp(self, h: ProtocolHarness) -> None:
        """Receiving signed datapoints should be handled."""
        data = pack(">HB", 0, 1) + self._dp_data()
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_RECEIVE_SIGN_DP, data
            )
        )
        h.notify(msg)
        await asyncio.sleep(0)
    ...
    async def test_receive_time_dp_type0(self, h: ProtocolHarness) -> None:
        """Receiving a string-encoded time datapoint should respond."""
        timestamp_bytes = b"1700000000123"
        data = pack(">B", 0) + timestamp_bytes + b"\x00\x00\x00" + self._dp_data()
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_RECEIVE_TIME_DP, data
            )
        )
        h.notify(msg)
        await asyncio.sleep(0)
    ...
    async def test_receive_time_dp_type1(self, h: ProtocolHarness) -> None:
        """Receiving an int-encoded time datapoint should be handled."""
        data = pack(">BI", 1, 1700000000) + self._dp_data()
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_RECEIVE_TIME_DP, data
            )
    ...
    async def test_receive_sign_time_dp(self, h: ProtocolHarness) -> None:
        """Receiving signed time datapoints should be handled."""
        data = pack(">HB", 0, 1) + pack(">B", 0) + b"1700000000123" + self._dp_data()
        msg = frame_packet0(
            encrypt_payload(
...
class TestDeviceInfo:
    """Tests for device info responses."""

    def _device_info_data(self) -> bytes:
        data = bytearray(46)
        data[0] = 1
        data[1] = 2
        data[2] = 3
    ...
    async def test_device_info_response(self, h: ProtocolHarness) -> None:
        """A device info response should populate the device attributes."""
        data = self._device_info_data()
        msg = frame_packet0(
            encrypt_payload(
                login_key(h), 4, 1, 0, TuyaBLECode.FUN_SENDER_DEVICE_INFO, data
            )
        )
        h.notify(msg)
    ...
    async def test_device_info_too_short(self, h: ProtocolHarness) -> None:
...
class TestPairResponse:
    """Tests for pairing responses."""

    async def test_pair_result_ok(self, h: ProtocolHarness) -> None:
        """A successful pairing result should mark the device as paired."""
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_SENDER_PAIR, b"\x00"
            )
        )
        h.notify(msg)
        assert h.device._is_paired is True

    async def test_pair_already_paired(self, h: ProtocolHarness) -> None:
        """An already-paired result should leave the device paired."""
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_SENDER_PAIR, b"\x02"
            )
        )
        h.notify(msg)
        assert h.device._is_paired is True

    async def test_pair_failed(self, h: ProtocolHarness) -> None:
        """A failed pairing result should clear the paired flag."""
        h.device._is_paired = True
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_SENDER_PAIR, b"\x01"
            )
        )
        h.notify(msg)
        assert h.device._is_paired is False

    async def test_pair_bad_length(self, h: ProtocolHarness) -> None:
...
class TestTimeRequests:
    """Tests for time request responses."""

    async def test_time1_request(self, h: ProtocolHarness) -> None:
        """A time1 request should produce a response."""
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_RECEIVE_TIME1_REQ, b""
            )
        )
        h.notify(msg)
        await asyncio.sleep(0)
    ...
    async def test_time2_request(self, h: ProtocolHarness) -> None:
        """A time2 request should produce a response."""
        msg = frame_packet0(
            encrypt_payload(
                session_key(h), 5, 1, 0, TuyaBLECode.FUN_RECEIVE_TIME2_REQ, b""
            )
        )
        h.notify(msg)
        await asyncio.sleep(0)
...
class TestValidation:
    ...
    async def test_unknown_code_ignored(self, h: ProtocolHarness) -> None:
    ...
    async def test_length_error(self, h: ProtocolHarness) -> None:
...
class TestFragmentation:
    """Tests for fragmented packet handling."""

    def _split_encrypted(self, encrypted: bytes, chunk: int = 12) -> list[bytes]:
        """Split encrypted bytes into chunks."""
        return [encrypted[i : i + chunk] for i in range(0, len(encrypted), chunk)]

    async def test_multiple_packets(self, h: ProtocolHarness) -> None:
        ...
        chunks = self._split_encrypted(encrypted)
        p0 = (
            pack_varint(0)
    ...
    async def test_missing_packet_resets(
        self, h: ProtocolHarness, caplog: pytest.LogCaptureFixture
    ) -> None:
    ...
    async def test_unexpected_length_resets(self, h: ProtocolHarness) -> None:
...
class TestErrorBranches:
    """Tests for error branches in the protocol."""

    async def test_send_packet_when_expected_disconnect(
        self, h: ProtocolHarness
    ) -> None:
    ...
    async def test_unpack_int_out_of_data(self, h: ProtocolHarness) -> None:
        """Unpacking an int from empty data should raise."""
        with pytest.raises(TuyaBLEDataFormatError):
            h.notify(b"\x80")

    async def test_unpack_int_too_many_bytes(self, h: ProtocolHarness) -> None:
        """Unpacking an oversized varint should raise."""
        with pytest.raises(TuyaBLEDataFormatError):
            h.notify(b"\x80\x80\x80\x80\x80")

    async def test_parse_timestamp_format_error(self, h: ProtocolHarness) -> None:
    ...
    async def test_parse_timestamp_length_error(self, h: ProtocolHarness) -> None:
    ...
    async def test_datapoints_invalid_type(self, h: ProtocolHarness) -> None:
    ...
    async def test_datapoints_data_length_error(self, h: ProtocolHarness) -> None:
    ...
    async def test_device_status_response_bad_length(self, h: ProtocolHarness) -> None:
    ...
    async def test_unexpected_packet_number(self, h: ProtocolHarness) -> None:
    ...
    async def test_get_key_invalid_security_flag(self, h: ProtocolHarness) -> None:
...
class TestPackInt:
    """Tests for _pack_int static method."""
...
class TestParseTimestamp:
    ...
    def test_time_type_zero(self) -> None:
    ...
    def test_time_type_one(self) -> None:
    ...
    def test_time_type_unknown_raises(self) -> None:
