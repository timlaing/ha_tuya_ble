...
class TestTuyaBLEDataPoint:
    """Tests for the TuyaBLEDataPoint class."""

    def test_properties(self, datapoints: TuyaBLEDataPoints) -> None:
        """Assert the data point exposes its stored properties."""
        dp = make_dp(
            datapoints, dp_id=7, dp_type=TuyaBLEDataPointType.DT_BOOL, value=True
        )
        assert dp.dp_id == 7
    ...
    def test_update_from_device_changed(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_update_from_device_unchanged(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_constructor_uses_update_from_device(
    ...
    async def test_set_value_enum_negative_raises(
        self, datapoints: TuyaBLEDataPoints
    ) -> None:
    ...
    def test_enum_positive_zero_set(self, datapoints: TuyaBLEDataPoints) -> None:
        """Assert the data point keeps its enum type after an update."""
        dp = make_dp(datapoints, dp_type=TuyaBLEDataPointType.DT_ENUM, value=1)
        assert dp.dp_type == TuyaBLEDataPointType.DT_ENUM


class TestTuyaBLEDataPoints:
    """Tests for the TuyaBLEDataPoints collection."""

    def test_len_and_getitem(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_get_or_create_existing(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_get_or_create_default_empty(self, datapoints: TuyaBLEDataPoints) -> None:
        """Assert a new data point defaults to an empty bytes value."""
        dp = datapoints.get_or_create(2, TuyaBLEDataPointType.DT_RAW)
        assert dp.value == b""

    def test_get_or_create_with_value(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_has_id(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_update_from_device_new(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    def test_update_from_device_existing(self, datapoints: TuyaBLEDataPoints) -> None:
    ...
    async def test_begin_end_update_sends_deferred(
        self, datapoints: TuyaBLEDataPoints, datapoints_owner: FakeDatapointsOwner
    ) -> None:
...
class TestDTString:
    """Tests for DT_STRING get_value and set_value."""

    def test_get_value_returns_encoded_string(self) -> None:
    ...
    async def test_set_value_converts_to_string(self) -> None:
