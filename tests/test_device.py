...
def test_properties_with_credentials() -> None:
...
def test_properties_without_credentials() -> None:
...
def test_name_uses_device_info_when_available() -> None:
...
def test_rssi_none_without_adv() -> None:
...
def test_datapoints_and_get_or_create() -> None:
...
def test_set_ble_device_and_advertisement_data() -> None:
...
def test_address_updates_with_ble_device() -> None:
...
async def test_initialize_with_credentials() -> None:
...
async def test_initialize_no_credentials() -> None:
...
async def test_update_device_info_reuses_existing() -> None:
...
def _adv(
    ...
    adv = FakeAdvertisementData(
        ...
        manufacturer_data={
            MANUFACTURER_DATA_ID: b"\x80\x02" + b"\x00" * 4 + _encrypt_uuid(product_id),
        },
    )
    return adv
...
async def test_stop_disconnects() -> None:
...
async def test_stop_with_done_tasks() -> None:
...
async def test_establish_connection_raises_not_found_returns_none() -> None:
...
async def test_establish_connection_bleak_error_returns_none() -> None:
...
async def test_disconnect_expected() -> None:
...
async def test_disconnect_unexpected_schedules_reconnect() -> None:
...
async def test_initialize_with_credentials_short_key_raises() -> None:
...
def test_build_pairing_request_no_device_info_raises() -> None:
...
def test_build_pairing_request_no_local_key_raises() -> None:
...
async def test_update_device_info_fetches_from_manager() -> None:
...
async def test_update_device_info_short_key_raises() -> None:
...
async def test_update_device_info_no_manager() -> None:
